...
if sys.platform == "win32":
    try:
        ...
        PYSIDE_RCC = os.path.join(PY_EXE_PATH, "Scripts", "pyside6-rcc.exe")
        assert os.path.exists(PYSIDE_RCC)
    except AssertionError:
        print("PySide6 is not found! PySide6 may be installed in a different location!")
        try:
            import PySide6

            PYSIDE_PKG_PATH = os.path.abspath(
                os.path.join(PySide6.__file__, os.pardir)
            )
            PYSIDE_RCC = os.path.join(PYSIDE_PKG_PATH, "pyside6-rcc.exe")
        except ImportError:
            print("PySide6 is not installed! Please install PySide6 first.")
elif sys.platform == "linux":
    try:
        PY_EXE_PATH = os.path.dirname(sys.executable)
        PYSIDE_RCC = os.path.join(PY_EXE_PATH, "pyside6-rcc")
        assert os.path.exists(PYSIDE_RCC)
    except AssertionError:
    ...
    for root, dirs, files in os.walk(rcDir):
        for basename in files:
            ...
            if fnmatch.fnmatch(basename, pattern):
                ...
                cmdStr = "{0} {1} -o {2}".format(PYSIDE_RCC, rcFilePath, pyFilePath)
                os.system(cmdStr)
